    ...
    return(
        <div className={styles.container}>
            ...
            <motion.div animate={product_animation} className="hidden opacity-0 flex-col items-center justify-center absolute top-0 left-0 w-screen h-screen bg-[#00000099]">
                <div className="w-1/2 rounded-lg bg-white shadow-xl flex flex-col items-center">
                    <div className="w-full px-2 flex flex-orw items-center justify-between">
                        <div className="text-lg font-semibold">Product:</div>
                        <FaTimes onClick={toggleProduct} className="text-lg cursor-pointer" />
                    </div>
                    <Slider images={selected_product.images}/>
                    <div className="w-11/12 flex flex-col items-center">
                        <div className="w-full font-semibold text-lg">{selected_product.name}</div>
                        <div className="w-full font-medium text-sm text-[#9a9a9b]">{selected_product.category.name}</div>
                        <div className="w-full font-semibold text-lg">${selected_product.price.value}</div>
                    </div>
