...
export async function getServerSideProps({ req,params }){
    ...
    var order_state = params.order_state;
    ...
    var orders = await getData('*[_type=="orders" && user->username==$username && state==$order_state]{_id,state,"products":products[]->{name,"images":images[].asset->url,price->,category->},payment_method,_createdAt}',{ username:user.username,order_state });
    
    return {
      props:{
        user:user_info[0],orders,order_state
      }
    }
  }

export default function MyOrders({user,orders,order_state}){
                        ...
                        <div className="w-full flex flex-row items-center">
                            {
                                ["Last orders","Processing","Shipped","Delivered"].map((value,index) => {
                                    return(
                                        <Link key={index} href={(value === "Last orders" ? "/my_profile/my_orders" : "/my_profile/my_orders/"+value)}><div className={"text-sm font-medium px-4 py-1 rounded-full bg-gray-100 shadow-md mr-4 cursor-pointer "+(order_state === value ? "text-blue-500" : "text-gray-600")}>{value}</div></Link>
                                    )
                                })
                            }
                        </div>
