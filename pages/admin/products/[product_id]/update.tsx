...
export default function UpdateProduct() {
    ...
    const { product, setProduct, isLoading, productImages, updateProduct } = useProduct(router.query.product_id as string);
    ...
    const updateProductHandler = async (event: React.MouseEvent<HTMLButtonElement>): Promise<void> => {
        ...
        if (result.isError) {
            ...
            toastManager.alertSuccess("Success", "Product Updated Successfully");
                    ...
                    <div className="w-11/12 flex flex-row">
                        <div className="w-1/2">
                            <div className="w-full mb-8">
                                ...
                                <input value={product.name} onChange={(event: React.ChangeEvent<HTMLInputElement>) => setProduct(p => ({ ...p, name: event.target.value }))} placeholder="Enter your porduct name..." className="bg-gray-100 text-black placeholder:text-gray-400 rounded-full w-full px-8 py-2 mt-2" type="text" />
                            </div>
                            <div className="w-full mb-8">
                                <div className="text-black text-lg font-medium">Price:</div>
                                <div className="flex items-center mt-2">
                                    <input value={product.price.value} onChange={(event: React.ChangeEvent<HTMLInputElement>) => setProduct(p => ({ ...p, price: { value: parseFloat(event.target.value), currency: p.price.currency } }))} placeholder="Enter your product price" className="bg-gray-100 text-black placeholder:text-gray-400 rounded-full flex-grow px-8 py-2" type="number" />
                                    <select value={product.price.currency} onChange={(event: React.ChangeEvent<HTMLSelectElement>) => setProduct(p => ({ ...p, price: { value: p.price.value, currency: event.target.value } }))} className="bg-gray-100 text-black placeholder:text-gray-400 rounded-full w-fit px-8 py-2 ml-2 outline-none border-r-8 border-gray-100 cursor-pointer">
                            ...
                            <div className="w-full mb-8">
                                ...
                                <select value={product.category._id} onChange={(event: React.ChangeEvent<HTMLSelectElement>) => setProduct(p => ({ ...p, category: { _id: event.target.value, name: event.target.value } }))} className="bg-gray-100 text-black placeholder:text-gray-400 rounded-full w-full px-8 py-2 outline-none border-r-8 border-gray-100 cursor-pointer mt-2">
                            ...
                            <div className="w-full my-16">
                                <button onClick={updateProductHandler} className="w-full bg-primary-color hover:bg-blue-600 rounded-full py-2 text-white duration-300 active:scale-105">Update</button>
                            </div>
                        </div>
